...
#MP.show()
NumberOfOneTraing = 880 #每單次訓練的使用的圖象數量
...
#    prediction_result = sess.run(tf.argmax(predictions, 1), feed_dict={x:test_xs})
#    print(prediction_result[:10])
#    tf.reset_default_graph()
    # Calculate accuracy for 256 mnist test images
    print("Testing Accuracy:",sess.run(accuracy, feed_dict={x: test_xs, y: test_ys, keep_prob: 1.}))
    print (type(tf.argmax(predictions, 1)))
    print(np.array(test_ys[0][0]))
