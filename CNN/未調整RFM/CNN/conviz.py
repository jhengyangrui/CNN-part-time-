...
# Reshape inputs
x_reshaped = tf.reshape(x, shape=[-1, 3, 4, 1])
#x_reshaped = tf.reshape(x, shape=[-1, 2, 4, 1])
# First convolutional layer
predictions = conv2d(x_reshaped, filter_sizex=4, filter_sizey=3, filter_num=7)

# First max-pooling layer
predictions = maxpool(predictions, 2, 1, 2, 1)

# Second convolutional layer
predictions = conv2d(predictions, filter_sizex=2, filter_sizey=3, filter_num=14)
...
#MP.show()
NumberOfOneTraing = 1584 #每單次訓練的使用的圖象數量
