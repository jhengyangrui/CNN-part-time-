...
def maxpool(x_, sizex, sizey, stridex, stridey):
    ...
    :return: tensor
    """
    return tf.nn.max_pool(x_,ksize=[1, sizey, sizex, 1],strides=[1, stridey, stridex, 1],padding='SAME')
...
predictions = conv2d(x_reshaped, filter_sizex=4, filter_sizey=7, filter_num=10)
...
predictions = fc(predictions, 8, keep_prob)
...
MP.show()
NumberOfOneTraing = 2288 #每單次訓練的使用的圖象數量
...
#    print(confusion_matrix(y, sess.run(tf.argmax(predictions, 1), feed_dict={x: test_xs,y: test_ys,keep_prob: 1.})))
#    sess.close()