...
# Network Parameters
n_input = 276  # MNIST data input (img shape: 28*28)
n_classes = 30  # MNIST total classes (0-9 digits)
...
def conv2d(x_, filter_sizex,  filter_sizey, filter_num, stride=1):
    ...
    activations = tf.nn.softmax(preactivations)
...
# Reshape inputs
x_reshaped = tf.reshape(x, shape=[-1, 69, 4, 1])

# First convolutional layer
predictions = conv2d(x_reshaped, filter_sizex=3, filter_sizey=2, filter_num=23)

# First max-pooling layer
predictions = maxpool(predictions, 2, 2, 2, 2)

# Second convolutional layer
predictions = conv2d(predictions, filter_sizex=3, filter_sizey=2, filter_num=46)

# Second max-pooling layer
predictions = maxpool(predictions, 2, 2, 2, 2)

# First fully-connected layer
predictions = fc(predictions, 200, keep_prob)
...
correct_predictions = tf.nn.in_top_k(predictions, targets, 3, name=None)
accuracy = tf.reduce_mean(tf.cast(correct_predictions, tf.float32))
tf_metric, tf_metric_update=tf.metrics.precision_at_top_k(y,predictions,3,class_id=None,weights=None,
                                                      metrics_collections=None,updates_collections=None,name="my_metric")
...
MP = PS()
MP.Addtest('./TraingData1000/')
#MP.show()
NumberOfOneTraing = 1384 #每單次訓練的使用的圖象數量

from test import PicSet as TPS

TMP = TPS()
TMP.Addtest('./TraingData1000/')
#MP.show()
TNumberOfOneTraing = 222
###########################################################
...
#                                                              y: batch_y,
#                                                              keep_prob: 1.})
#            print("\rIter " + str(step*batch_size) + ", Minibatch Loss= " +"{:.6f}".format(loss) + ", Training Accuracy= " +"{:.5f}".format(acc), end='')
#        step += 1
    ...
    c,a =  sess.run([predictions,accuracy], feed_dict={x: test_xs,
                                        y: test_ys,
                                        keep_prob: 1.})
    '''
